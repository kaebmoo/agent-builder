...
- Atlas: `atlas-control-plane` ใช้ `POST /agent/run` เท่านั้น; node field ที่รองรับ: `role, tags, workspace_id, model, output_format, collect_files`; `output_format: json` ทำ `json.loads` ทั้งก้อน ไม่ตรวจ schema
