...
ลำดับตาม baseline ที่ตกลง: schema → fixtures → matrix → generator → thclaws validate → static audit → live audit → export → standalone generator (M9, เพิ่ม 2026-09-07 ต่อท้าย MVP เพื่อไม่เลื่อนสาย Atlas)
...
M9 (เพิ่ม 2026-09-07): standalone generator เริ่มจาก `static-pipeline` เท่านั้น ใช้ 3 role คงที่ตาม scaffold ของ thClaws โดยไม่ขยาย schema; ต้องเพิ่ม fixture standalone ก่อน และ M5/M6 ต้องผูกกฎ single-worker กับ target Atlas ไว้ล่วงหน้า ดู DESIGN §8 เรื่อง permission ต่อ role
